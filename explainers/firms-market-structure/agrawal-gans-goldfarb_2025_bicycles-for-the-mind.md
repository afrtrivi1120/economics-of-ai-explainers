...
- **Bet on the complement, not the substitute.** The framework predicts that generative AI, in its current phase, is closing productivity gaps within firms — which is an opportunity for workers without elite credentials (junior analysts, physicians in under-resourced settings, teachers, lawyers at small firms) to access tasks today reserved for experts. This reinforces the "let non-elites do expert tasks" framing from Autor.
- **Design authority, not just tool delivery.** The key result from the medical example (p. 11) — "AI should be prioritized for physicians who have the authority and knowledge to act" — implies that regulations on prescription, professional scope, and function delegation determine how much value AI generates. A world-class diagnostic AI creates no value if the technician operating it cannot act on the output. Policymakers should align professional-scope rules with AI deployment strategies.
- **Do not expect full automation.** Public-sector bodies evaluating whether to replace workers with AI should heed the model's warning: the rigidity of pre-specified judgment makes total automation costly wherever context varies (citizen services, administrative justice, health). The dominant path is hybrid human-tool collaboration.
- **Watch the inequality U-turn.** Policymakers who adopt generative AI today and capture the inverse skill bias phase may reduce within-firm gaps — but the model predicts that with more mature tools, "opportunity judgment" (creativity, problem definition) will again command a premium, which requires parallel investment in higher-order education.
